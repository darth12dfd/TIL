...
<img width="328" height="248" alt="image" src="https://github.com/user-attachments/assets/7669cdd8-fcf5-4741-9a6b-7b5e0965c992" />
...
<img width="913" height="205" alt="image" src="https://github.com/user-attachments/assets/49c751e7-4e2b-4f91-b0f3-ef1e65e0ef81" />
...
* 전위 순회(preorder)와 중위 순회(inorder)를 이용해 트리를 복원한 뒤, 후위 순회(postorder) 결과를 출력하는 전형적인 재귀 + 분할정복 문제
...
* 전위 순회 배열에서 첫 번째 원소는 현재 서브트리의 루트. 이 루트를 중위 순회에서 찾아서 좌/우 서브트리를 분리할 수 있음.

### 작동 원리

* 전위 순회 배열의 첫 원소는 루트 → preorder[pLo]

* 그 루트를 중위 순회에서 찾으면
  → 왼쪽 노드 수 = mid - iLo
  → 오른쪽 노드 수 = iHi - mid

* 왼쪽 서브트리 → 오른쪽 서브트리 → 루트 순으로 재귀 호출하며 후위 순회 결과를 만듦.
...
fun main() {
    ...
    repeat(t) {
        ...
        val pos = IntArray(n + 1) // 특정 노드의 inorder 위치 저장
        ...
        fun buildPost(
            ...
            // 현재 서브트리의 루트
            val root = preorder[pLo]
            // 중위 순회에서 root의 인덱스
            val mid = pos[root]
            // 왼쪽 서브트리 크기
            val leftSize = mid - iLo

            // 왼쪽 서브트리
            buildPost(pLo + 1, pLo + leftSize, iLo, mid - 1)
            // 오른쪽 서브트리
            buildPost(pLo + leftSize + 1, pHi, mid + 1, iHi)
            // 후위 순회: 왼쪽, 오른쪽 처리 후 루트 출력
            sb.append(root).append(' ')
        }

        // 전체 범위로 호출
        buildPost(0, n - 1, 0, n - 1)
        sb.append('\n')
    }
