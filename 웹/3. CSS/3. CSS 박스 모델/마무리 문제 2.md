...
* 위의 소스코드에는 ```<section>``` 태그 안에 ```<article>``` 태그 4개와 ```<footer>``` 태그 1개가 포함되어 있다. 다음 조건을 만족하면서 article 요소를 격자 형태로 배치하도록하는 article 타입 스타일을작성하라.

```
1. article 요소를 왼쪽으로 플로팅한다.
2. article 요소의 너비는 350px, 높이는 200px, 마진과 패딩은 각각 10px로 한다.
3. article 요소 주변에 1px짜리 회색(#ccc) 실선을 그린다.
4. footer 요소에 왼쪽 플로팅을 해제한다.
```
